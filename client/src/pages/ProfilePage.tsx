...
import { ArrowLeft, User, Mail, Shield, Award, Edit3, Key, Camera, Loader2, CheckCircle2, AlertCircle, X } from 'lucide-react'
...
export const ProfilePage = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Here we'll convert to Base64 for the mock backend/demo.
            ...
            <div className="glass-card p-8 bg-white flex flex-col items-center text-center shadow-sm relative overflow-hidden group">
              ...
              <div className="relative mb-6">
                ...
                <button 
                  ...
                  className="absolute bottom-[-8px] right-[-8px] w-10 h-10 rounded-2xl bg-[#0f172a] text-white flex items-center justify-center border-4 border-white shadow-lg hover:scale-110 active:scale-95 transition-transform"
                >
                  <Camera size={18} />
                </button>
              ...
              <p className="text-xs font-bold text-[var(--color-text-muted)] flex items-center gap-1.5 justify-center mb-6">
                <Mail size={12} /> {user?.email}
              </p>
              
              <div className="w-full pt-6 border-t border-gray-100 flex flex-col gap-3 text-xs font-bold">
                <div className="flex justify-between items-center px-2">
                  <span className="text-gray-400 flex items-center gap-2 uppercase tracking-widest text-[9px]"><Shield size={12}/> Role</span>
                  <span className="text-[#0f172a] bg-gray-50 px-3 py-1 rounded-lg border border-gray-100 capitalize">{user?.role}</span>
                </div>
                <div className="flex justify-between items-center px-2">
                  <span className="text-gray-400 flex items-center gap-2 uppercase tracking-widest text-[9px]"><Award size={12}/> Status</span>
                  <span className="text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg border border-emerald-100">Active</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-2 flex flex-col gap-6">
            <div className="glass-card p-8 bg-white shadow-sm border border-gray-100/50">
              <h3 className="text-lg font-bold text-[#0f172a] mb-6 flex items-center gap-2">
                <User size={20} className="text-[var(--color-primary)]" /> Personal Information
              </h3>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full pl-11 pr-4 py-3 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-semibold text-[#0f172a] focus:outline-none focus:border-[var(--color-primary)] focus:bg-white transition-all disabled:opacity-60" 
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full pl-11 pr-4 py-3 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-400 focus:outline-none" 
                    />
                  </div>
                  <p className="text-[10px] font-bold text-gray-300 mt-2 px-1">Email cannot be changed for security reasons.</p>
                </div>
                  ...
                  {isEditing ? (
                    <>
                      <button 
                        ...
                        className="px-5 py-2.5 text-sm font-bold text-gray-500 hover:text-[#0f172a]"
                      >
                        Cancel
                      </button>
                      <button 
                        type="submit"
                        disabled={isLoading}
                        className="liquid-glass-btn px-6 py-2.5 rounded-xl text-sm font-bold text-white flex items-center gap-2 shadow-lg shadow-[var(--color-primary-glow)]"
                      >
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 text-sm font-bold text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-white transition-all px-5 py-2.5 rounded-xl border border-[var(--color-primary)]/20"
                    >
            ...
            <div className="glass-card p-8 bg-white shadow-sm border border-gray-100/50">
              <h3 className="text-lg font-bold text-[#0f172a] mb-2 flex items-center gap-2">
                <Key size={20} className="text-[var(--color-primary)]" /> Security Settings
              </h3>
              <p className="text-sm font-medium text-[var(--color-text-muted)] mb-6">Keep your account secure by using a strong password.</p>
              
              <button 
                onClick={() => setShowPwModal(true)}
                className="flex items-center gap-3 text-sm font-bold text-[#0f172a] px-6 py-3 rounded-2xl border border-gray-100 hover:border-[var(--color-primary)] bg-gray-50 hover:bg-white hover:shadow-lg hover:shadow-[var(--color-primary-glow)]/10 transition-all group"
              >
                <div className="w-8 h-8 rounded-lg bg-white border border-gray-100 flex items-center justify-center text-gray-400 group-hover:text-[var(--color-primary)] transition-colors">
                  <Key size={16} />
                </div>
                Change Password
              </button>
        ...
        {showPwModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
            ...
            <motion.div 
              ...
              className="w-full max-w-md bg-white rounded-[2.5rem] p-10 shadow-2xl relative z-10 border border-white"
            >
              <h2 className="text-2xl font-extrabold text-[#0f172a] mb-2">Change Password</h2>
              <p className="text-sm font-medium text-gray-500 mb-8">Enter your current password to set a new one.</p>
              ...
              <form onSubmit={handlePasswordChange} className="space-y-5">
                ...
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-5 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold focus:outline-none focus:border-[var(--color-primary)]" 
                  />
                </div>
                <div className="pt-2">
                ...
                <div className="flex gap-4 pt-6">
                  <button 
                    type="button" 
                    onClick={() => setShowPwModal(false)}
                    className="flex-1 py-4 text-sm font-bold text-gray-500 hover:text-[#0f172a]"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit"
                    disabled={isLoading}
                    className="flex-1 liquid-glass-btn py-4 rounded-[1.5rem] text-sm font-bold text-white shadow-lg shadow-[var(--color-primary-glow)]"
                  >
                    {isLoading ? <Loader2 size={18} className="animate-spin m-auto" /> : 'Update'}
                  </button>
                </div>
              </form>
            </motion.div>
